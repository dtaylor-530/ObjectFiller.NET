...
namespace Tynamix.ObjectFiller
{
    ...
    /// </summary>
    /// <typeparam name="TTargetObject">Type of the object for which this setup is related to</typeparam>
    /// <typeparam name="TProperty">Type of the property which will be setup</typeparam>
    public class FluentPropertyApi<TProperty> : IFluentApi<TProperty>
    {
        /// <summary>
        /// The affected properties
        /// </summary>
        ...
        /// The callback function
        /// </summary>
        private readonly FluentFillerApi callback;
        ...
        /// The object filler setup manager
        /// </param>
        internal FluentPropertyApi(IEnumerable<PropertyInfo> affectedProperties, FluentFillerApi callback, SetupManager setupManager, Type type)
        {
            this.affectedProperties = affectedProperties;
            this.callback = callback;
            this.setupManager = setupManager;
        ...
        /// <param name="propertyOrder">Defines if the property will be filled at "TheBegin" or at "TheEnd"</param>
        /// <returns>FluentPropertyAPI to define which </returns>
        public FluentPropertyApi<TProperty> DoIt(At propertyOrder)
        {
            foreach (PropertyInfo propertyInfo in this.affectedProperties)
            {
                this.setupManager.GetFor(Type).PropertyOrder[propertyInfo] = propertyOrder;
            }
        ...
        /// </summary>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi UseDefault()
        {
            return this.callback;
        }
        ...
        /// <param name="valueToUse">Value which will be used</param>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi Use(TProperty valueToUse)
        {
            return this.Use(() => valueToUse);
        }
        ...
        /// <param name="randomizerFunc"><see cref="Func{TTargetType}"/> which will be used to generate a value of the <typeparamref name="TTargetType"/></param>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi Use(Func<TProperty> randomizerFunc)
        {
            foreach (PropertyInfo propertyInfo in this.affectedProperties)
            {
                this.setupManager.GetFor(Type).PropertyToRandomFunc[propertyInfo] = () => randomizerFunc();
            }
        ...
        /// </summary>
        /// <param name="setup">The setup which is used for configuration.</param>
        public FluentFillerApi Use(FillerSetup setup)
        {
            foreach (PropertyInfo propertyInfo in this.affectedProperties)
            {
                var factoryMethod = Randomizer<TProperty>.CreateFactoryMethod(setup);
                this.setupManager.GetFor(Type).PropertyToRandomFunc[propertyInfo] = () => factoryMethod();
            }
        ...
        /// <param name="randomizerPlugin">A <see cref="IRandomizerPlugin{TTargetType}"/> which will be used to generate a value of the <typeparamref name="TTargetType"/></param>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi Use(IRandomizerPlugin<TProperty> randomizerPlugin)
        {
            return this.Use(randomizerPlugin.GetValue);
        }
        ...
        /// <typeparam name="TRandomizerPlugin">A <see cref="IRandomizerPlugin{TTargetType}"/> which will be used to generate a value of the <typeparamref name="TTargetType"/></typeparam>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi Use<TRandomizerPlugin>()
            where TRandomizerPlugin : IRandomizerPlugin<TProperty>, new()
        {
            this.Use(new TRandomizerPlugin());
            return this.callback;
        }
        ...
        /// <param name="enumerable">An <see cref="IEnumerable{TTargetType}"/> with items of type <typeparamref name="TTargetObject"/> which will be used to fill the data.</param>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi Use(IEnumerable<TProperty> enumerable)
        {
            return this.Use(new EnumeratorPlugin<TProperty>(enumerable));
        }
        ...
        /// </summary>
        /// <returns>Main FluentFiller API</returns>
        public FluentFillerApi IgnoreIt()
        {
            foreach (PropertyInfo propertyInfo in this.affectedProperties)
            {
                this.setupManager.GetFor(Type).PropertiesToIgnore.Add(propertyInfo);
            }
