...
namespace Tynamix.ObjectFiller
{
    ...
    /// and <see cref="FluentPropertyApi{TTargetObject,TTargetType}"/>. It provides the common methods for both.
    /// </summary>
    /// <typeparam name="TProperty">Type which will be setup</typeparam>
    internal interface IFluentApi<TProperty>
    {
        /// <summary>
        /// Defines which static value will be used for the given <typeparamref name="TProperty"/>
        /// </summary>
        /// <param name="valueToUse">Value which will be used</param>
        /// <returns>Main FluentFiller API</returns>
        FluentFillerApi Use(TProperty valueToUse);

        /// <summary>
        /// Defines which <see cref="Func{TResult}"/> will be used to generate a value for the given <typeparamref name="TProperty"/>
        /// </summary>
        /// <param name="randomizerFunc"><see cref="Func{TTargetType}"/> which will be used to generate a value of the <typeparamref name="TProperty"/></param>
        /// <returns>Main FluentFiller API</returns>
        FluentFillerApi Use(Func<TProperty> randomizerFunc);

        /// <summary>
        /// Defines which implementation of the <see cref="IRandomizerPlugin{T}"/> interface will be used to generate a value for the given <typeparamref name="TProperty"/>
        /// </summary>
        /// <param name="randomizerPlugin">A <see cref="IRandomizerPlugin{TTargetType}"/> which will be used to generate a value of the <typeparamref name="TProperty"/></param>
        /// <returns>Main FluentFiller API</returns>
        FluentFillerApi Use(IRandomizerPlugin<TProperty> randomizerPlugin);
        ...
        /// <param name="enumerable">An <see cref="IEnumerable{TTargetType}"/> with items of type <typeparamref name="TTargetObject"/> which will be used to fill the data.</param>
        /// <returns>Main FluentFiller API</returns>
        FluentFillerApi Use(IEnumerable<TProperty> enumerable);

        /// <summary>
        /// Ignores the entity for which the fluent setup is made (Type, Property)
        /// </summary>
        /// <returns>Main FluentFiller API</returns>
        FluentFillerApi IgnoreIt();
    }
}