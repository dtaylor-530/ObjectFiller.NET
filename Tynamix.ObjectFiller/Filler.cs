...
#pragma warning disable CS0419 // Ambiguous reference in cref attribute
        ...
        /// <summary>
        /// The type of the objects that are being created via <see cref="Create"/>
        /// </summary>
...
#pragma warning restore CS0419 // Ambiguous reference in cref attribute
        ...
        public object Create()
        {
            ...
            if (!TypeIsClrType(Type))
            {
                objectToFill = this.CreateInstanceOfType(Type, this.setupManager.GetFor(Type), hashStack);
                this.Fill(objectToFill);
            }
            else
            {
                objectToFill = this.CreateAndFillObject(Type, this.setupManager.GetFor(Type), hashStack);
            }
        ...
        /// The filled instance of dictionaryType <typeparamref name="T"/>.
        /// </returns>
        public object Fill(object instanceToFill)
        {
            this.FillInternal(instanceToFill);
        ...
        /// <summary>
        /// Call this to start the setup for the <see cref="Filler"/>
        /// </summary>
        /// <returns>Fluent API setup</returns>
        public FluentFillerApi Setup()
        {
            return this.Setup(false);
        }
        ...
        /// <param name="explicitSetup">True if just properties shall get filled which configured in filler setup.</param>
        /// <returns>Fluent API setup</returns>
        public FluentFillerApi Setup(bool explicitSetup)
        {
            return this.Setup(null, explicitSetup);
        }
        ...
        /// Fluent API Setup
        /// </returns>
        public FluentFillerApi Setup(FillerSetup fillerSetupToUse, bool explicitSetup)
        {
            if (fillerSetupToUse != null)
            {
                this.setupManager.FillerSetup = fillerSetupToUse;
            ...
            this.justConfiguredProperties = explicitSetup;
            return new FluentFillerApi(this.setupManager, Type);
        }
        ...
        /// <param name="seed">Number for the data generation</param>
        /// <returns>This ObjectFiller instance</returns>
        public Filler SetRandomSeed(int seed)
        {
            Random.SetSeed(seed);
            return this;
        }
        ...
        /// Throws exception if the constructor could not be created by filler setup
        /// </exception>
        private object CreateInstanceOfType(Type type, FillerSetupItem currentSetupItem, HashStack<Type> hashStack)
        {

            var constructorArgs = ConstructorArgumentTypes()
                .Select(t => this.CreateAndFillObject(Type, this.setupManager.GetFor(Type), hashStack))
                .ToArray();

            object result = Activator.CreateInstance(type, constructorArgs);
            return result;

            ICollection<Type> ConstructorArgumentTypes()
            {
                const string InvalidConstructor = "Could not found a constructor for type [{0}] where the parameters can be filled with the current objectfiller setup";
                List<Exception> invalidTypeExceptions = new List<Exception>();
                foreach (var types in ConstructorParameterTypes())
                {
                    List<Type> validTypes = new List<Type>();

                    foreach (var ctorParamType in types.Value)
                    {               
                        if (ctorParamType == type)
                        {
                            invalidTypeExceptions.Add(new InvalidParameterTypeException { 
                                Reason = InvalidParameterTypeException.ExceptionReason.CircularReference, 
                                ConstructorInfo = types.Key, 
                                ParameterType = ctorParamType,
                                Type = type
                            });
                        }
                        else if (TypeIsValidForObjectFiller(ctorParamType, currentSetupItem))
                        {
                            validTypes.Add(ctorParamType);
                        }
                        else
                        {
                            invalidTypeExceptions.Add(new InvalidParameterTypeException {
                                Reason = InvalidParameterTypeException.ExceptionReason.TypeIsInValid,
                                ConstructorInfo = types.Key, 
                                ParameterType = ctorParamType,
                                Type = type
                            });
                    ...
                    if (validTypes.Count == types.Value.Length)
                        return validTypes;
                }

                if (invalidTypeExceptions.Count > 0)
                    throw new CombinedException(string.Format(InvalidConstructor, type.Name)) { Exceptions = invalidTypeExceptions };
                return
                    new List<Type>() { };

                IEnumerable<KeyValuePair<ConstructorInfo, Type[]>> ConstructorParameterTypes() =>
                    from ctorInfo in type.GetConstructors()
                    let length = ctorInfo.GetParameters().Length
                    where length != 0
                    orderby length
                    let paramTypes = ctorInfo.GetParameters().Select(p => p.ParameterType).ToArray()
                    select new KeyValuePair<ConstructorInfo, Type[]>(ctorInfo, paramTypes);
            }
        }
        ...
        {
            public CombinedException(string message) : base(message)
            {
            }
